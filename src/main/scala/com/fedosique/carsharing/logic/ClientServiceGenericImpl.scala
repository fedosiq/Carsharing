...
class ClientServiceGenericImpl[F[_]: Monad, DbEffect[_]: Monad](carStorage: CarStorage[DbEffect],
                                                               ...
                                                               (implicit evalDb: DbEffect ~> Future, ec: ExecutionContext) extends ClientService[Future] {

  override def getCar(id: UUID): Future[Option[Car]] = evalDb(carStorage.get(id).map(_.filterNot(_.status.isOccupied)))

  override def availableCars(loc: Location): Future[Seq[Car]] =
  ...
  override def occupyCar(carId: UUID, userId: UUID): Future[Car] = getCar(carId).flatMap {
  ...
  override def leaveCar(carId: UUID, userId: UUID): Future[Car] = evalDb(carStorage.get(carId).flatMap {
